...
if (isset($_POST['login'])) {
      $username = filter_var($_POST['SMName'], FILTER_SANITIZE_STRING);
      $password_1 = $_POST['password'];
      //First hash the password
      $query = "SELECT * FROM supportmanager WHERE name = :name";
          ...
          if (password_verify($password_1,$password)) {
           ...
           // echo "<br>no<br>";
            ...
            header('Location:../views/smlogin.php');
...
//logout
    ...
    if (isset($_POST['changePassword'])) {
      ...
      $_SESSION["msg_type"] = "danger";
      header('Location:../views/smsettings.php');

    }else {
      // code...
      $hashed_password = password_hash($confirmPassword, PASSWORD_DEFAULT);

      $stmt = $conn ->prepare("UPDATE supportmanager SET password = :confirmPassword WHERE id=$USER_ID");
    ...
    if (isset($_POST['remove'])){
      $delete_id = $_POST['id'];
      $stmt = $conn ->prepare("DELETE FROM supportmanager WHERE id = :id");
      $stmt->bindParam(':id', $delete_id);
      $stmt->execute();

      //REDIRECT THE USER
      header("Location:../views/smaccount.php");
    }
